...
A simple idea: automatically download your YouTube Watch Later playlist, extract transcripts, summarize with local AI, organize by topic, store in your Obsidian vault. Daily automation. Set it and forget it.
...
The entire premise assumes you use YouTube's "Watch Later" playlist. It's where you save videos to watch later. Simple, right?

The YouTube Data API returns 0 items for Watch Later. Always.

This is a known limitation: Watch Later doesn't sync properly with the API. It's not an authentication problem or a code bug. YouTube simply doesn't expose your Watch Later list through the API in a reliable way.
...
Your actual Obsidian vault is accessible via the Obsidian MCP, but the script writes directly to the filesystem instead of using it. This creates files in a location that:
- Isn't integrated with your actual vault
- Isn't visible in Obsidian
- Isn't backed up with your vault
- Defeats the purpose of "organized by topic in your Obsidian vault"
...
You then have files like `Other.md` (because the title was empty and fell back to "Other" topic) with no indication of what video it actually came from.
...
The more stages you have, the more likely at least one fails. This system requires:
- YouTube API working
- yt-dlp working
...
If these models aren't running on the OLLAMA server, everything fails silently. You get timeout errors with no visibility into why.
...
Every external service (YouTube API, OLLAMA server, Obsidian MCP) is a failure point. And failures are often silent—you get a timeout, not a helpful error message.

### Learning 5: Configuration Shouldn't Assume Knowledge

Telling a system to use `qwen2.5:14b` model is meaningless if:
- You don't know if that model is loaded
- You don't have visibility if it fails
- You can't debug why it's slow
...
**If you run the current system:**
- It might fetch your playlists (depends on which fetch script is actually executing)
...
**Meta-Learning:** Sometimes the most valuable lesson from a project isn't the thing you built. It's understanding why it didn't work, what you assumed incorrectly, and what you'd do differently next time.

The YouTube queue will eventually work. But declaring it done and moving on means it'll still be broken in three months when you finally try to use it.
